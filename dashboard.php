...
@section('nav-links')
    ...
    <a href="{{ url('/student/register') }}"><i class="nav-icon">✎</i> Enrollment Form</a>
    <a href="{{ url('/admin/dashboard') }}" class="active"><i class="nav-icon">▤</i> Dashboard</a>
    <a href="{{ url('/admin/exams') }}"><i class="nav-icon">📝</i> Entrance Exams</a>
    <a href="{{ url('/admin/invites') }}"><i class="nav-icon">✉</i> Invites</a>
    <a href="{{ url('/admin/fees') }}"><i class="nav-icon">💳</i> Billing</a>
    <a href="{{ url('/admin/notices') }}"><i class="nav-icon">📣</i> Notices</a>
    <a href="{{ url('/admin/class-levels') }}"><i class="nav-icon">🏫</i> Classes</a>
    <a href="{{ url('/admin/teachers') }}"><i class="nav-icon">🧑🏫</i> Teachers</a>
    <a href="{{ url('/admin/exemplars') }}"><i class="nav-icon">⭐</i> Exemplars</a>
    <a href="{{ url('/admin/settings') }}"><i class="nav-icon">🎨</i> Settings</a>
@endsection

@section('topbar-right')
    <span class="user-greeting">🔑 <strong>{{ auth('admin')->user()->username ?? auth('admin')->user()->email ?? 'Admin' }}</strong></span>
    <a href="{{ url('/student/register') }}" class="btn-gold" style="padding:8px 18px; font-size:12px;">+ New Enrollment</a>
...
@section('content')
    ...
    <form method="GET" action="{{ url('/admin/dashboard') }}" class="filters">
        ...
        <div class="filter-actions">
            ...
            <a href="{{ url('/admin/dashboard') }}" class="btn-reset">↺ Reset</a>
        </div>
    </form>
                ...
                @forelse ($students as $i => $student)
                    <tr>
                        ...
                        <td style="white-space:nowrap;">
                            <a href="{{ url('/admin/students/'.$student->id) }}" class="view-btn">View</a>
                            <form method="POST" action="{{ url('/admin/students/'.$student->id) }}" style="display:inline;" onsubmit="return confirm('Delete this applicant?');">
                                @csrf
                                @method('DELETE')
            ...
            <div style="display:grid; grid-template-columns: 1fr 1fr; gap:12px;">
                @foreach ($recentApplicants as $app)
                    <div style="display:flex; align-items:center; gap:12px; padding:10px 14px; background:var(--cream); border-radius:8px; border:1px solid var(--border);">
                        ...
                        <a href="{{ url('/admin/students/'.$app->id) }}" class="view-btn" style="font-size:10px;">View</a>
                    </div>
                @endforeach
            </div>
        @else
    ...
    <div class="card card-padded" style="margin-bottom:24px;">
        ...
        <form method="POST" action="{{ url('/activities') }}" style="display:grid; grid-template-columns: 2fr 1fr 1fr auto; gap:12px; align-items:end; margin-bottom:20px;">
            @csrf
            <div>
            ...
            <tbody>
                @forelse ($allActivities as $activity)
                    <tr>
                        ...
                        <td>
                            <form method="POST" action="{{ url('/activities/'.$activity->id) }}" style="display:inline;" onsubmit="return confirm('Delete this activity?');">
                                @csrf
                                @method('DELETE')
